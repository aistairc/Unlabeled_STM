function preprocessing_ds3(config)
...
for sub_ind = 1:length(folders)
    ...
    for file_ind = 1:length(files)
        ...
        for k = 1:2:length(change_point)
            ...
            feat = extract_feature(seg, analysis_win, shift_win);
    ...
    disp(['preprocess dataset 3 sub ', num2str(sub_ind), ' done']);
end
